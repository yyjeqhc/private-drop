...
use super::audit::audit_policy_for_tool;
...
use webcodex_tool_contracts::audit_policy::{AuditExecutionPolicy, AuditTransform};
...
pub fn validation_output_summary_for_tool_result(tool_name: &str, output: &Value) -> Option<Value> {
    ...
    if policy.execution == AuditExecutionPolicy::Omit {
        return None;
    }
    ...
    let mut summary = json!({
        ...
        "shell": output
            ...
            .cloned()
            .unwrap_or_else(|| {
                match policy.request.transform {
                    AuditTransform::ProcessExecution => Value::String("direct_argv".to_string()),
                    AuditTransform::ScriptExecution => output.get("language").cloned().unwrap_or(Value::Null),
                    _ => Value::Null,
                }
            }),
    ...
    if matches!(
        policy.execution,
        AuditExecutionPolicy::TestCounts | AuditExecutionPolicy::TestAssertions
    ) {
    ...
    if policy.execution == AuditExecutionPolicy::TestAssertions {
