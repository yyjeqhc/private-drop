...
pub fn agent_continuation_app_tool_specs() -> Vec<ToolSpec> {
    vec![
        tool_spec(
            ...
            "App-only bind of one live View to one exact freshly attached Durable Agent Endpoint generation. Supply a stable binding_id generated by the View using 128 secure random bits (wc_host_binding_ plus 32 lowercase hex). It is a process-local fence, never authority. Re-authorizes communication:manage on every call. Retry a lost response with the same exact binding_id: the current same-View carrier renews without revoking claims or resetting dispatch. A different View replaces the older carrier using existing pre/post-fence reconciliation. No durable continuation truth is created.",
