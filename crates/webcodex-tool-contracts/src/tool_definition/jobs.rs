...
pub(super) const EXECUTION_DEFINITIONS: &[ToolDefinition] = &[
    ...
    model_spec(
        ...
        "Run bounded sh, bash, PowerShell, JavaScript, or TypeScript content as typed script data from a Runner-owned file. JavaScript is Node.js-backed with fixed .mjs ESM semantics. TypeScript is Node.js native erasable type stripping from a Runner-owned .mts ESM file and requires Node.js 22.6.0 or newer; it supports TypeScript constructs that can be erased, such as type annotations, interfaces, type aliases, and generics, but it does not type-check, read tsconfig.json as a project compiler, or support transform-required syntax such as enum or parameter properties. The Runner resolves node from its prepared execution environment and owns any compatibility flags. Callers cannot choose a runtime executable/path or runtime flags. WebCodex does not install npm dependencies, run tsc, bootstrap a package manager, or fall back to Bun, Deno, tsx, or another runtime. Relative ESM imports resolve from the Runner-owned temporary module, not project cwd; use Node builtins or explicit project paths/file URLs for project code. Prefer run_process for one short native executable with literal argv, run_script for program-like scripts such as loops, branches, functions, async/await, or multi-stage data/text/filesystem processing, and run_shell when shell grammar is required. Script args remain literal native argv and stdin remains independent. Long work continues as the same execution / same Job after handoff and is never restarted; the script body never becomes shell command text. If work must outlive the current Runner process, use a native executable and discover run_detached_process instead.",
