    ...
    transaction
        .execute(
            "UPDATE wc_agent_task_endpoint_executions
             ...
             WHERE wake_id IN (
                 SELECT wake_id FROM wc_agent_wakes
                 WHERE target_agent_id = ?3 AND state = 'claimed'
                   AND claimed_endpoint_id = ?1 AND claimed_controller_generation = ?2
...
fn wake_envelope(
        ...
        == WAKE_TRIGGER_AGENT_TASK_ATTEMPT
    {
        ...
        let (title, instruction, attempt_fence, attempt_controller_generation): (
            String,
            String,
            String,
            i64,
            ...
            .query_row(
                "SELECT t.title, t.instruction, a.attempt_fence, a.attempt_controller_generation
                     FROM wc_agent_tasks t
                     JOIN wc_agent_task_attempts a ON a.task_id = t.task_id
                     WHERE t.task_id = ?1 AND a.attempt_id = ?2",
        ...
        let resume_hint = format!(
                "This is an exact WebCodex Durable AgentTask continuation.\n\nagent_id={}\nendpoint_id={}\ncontroller_generation={}\nwake_id={}\nconsume_token={}\ntask_id={}\nattempt_id={}\nattempt_fence={}\nattempt_controller_generation={}\n\nTask title:\n{}\n\nTask instruction:\n{}\n\nAuthoritative continuation contract:\n1. First call bootstrap_agent_conversation with this exact agent_id, endpoint_id, controller_generation, and wake_id. The returned Wake source must remain agent_task_attempt with the exact task_id and attempt_id above; never infer or retarget identities from ambient Host, Project, Workflow Session, ClientWindow, Goal, credential, recent Agent, or recent Task state.\n2. Continue only this exact leased AgentTaskAttempt. Use the exact attempt_fence and attempt_controller_generation above for heartbeat/completion operations. The TaskAttempt lease is independent from the Endpoint lease; if the Attempt is stale or expired, fail closed and do not revive it.\n3. Read the authoritative AgentTask if more task metadata is needed. This Host continuation was created from durable Task truth and intentionally creates no synthetic Conversation Message or Inbox Delivery.\n4. Consume this exact Wake with consume_agent_wake only after this model turn has actually taken over the continuation. Wake consumption is separate from AgentTask completion.\n5. Agent/Conversation authority does not grant Project, Runner, filesystem, Goal, or Workflow Session authority. If task execution needs those capabilities, use the ordinary WebCodex authorization/project workflow and the Task's explicit references.\n6. After processing the durable work, make this resumed turn's user-visible final response reflect the actual result or blocker; do not merely restate this continuation contract.\n",
                wake.target_agent_id,
                endpoint_id,
                controller_generation,
                wake.wake_id,
