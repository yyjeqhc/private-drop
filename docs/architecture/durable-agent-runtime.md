...
- process-local Host bindings are empty after restart; offline Messages, Deliveries,
  ...
  register a carrier. An already-open MCP App may recreate only its lost local
  binding when its durable SHA-256 recovery fingerprint still names that exact
  current View/Endpoint generation after takeover.
...
G3 adds an optional production ChatGPT MCP App Host carrier on top of this substrate. It is deliberately a pull bridge rather than a fake Server callback: one explicit `present_agent_continuation(agent_id, endpoint_id, expected_controller_generation)` card binds the current `ui://webcodex/agent-continuation/v12` resource, while ModelHidden app-only operations establish one exact process-local View binding, renew the exact Endpoint, acquire through the existing Wake claim state machine, cross the existing durable dispatch fence, and record Host dispatch acceptance or uncertainty. The View itself performs `ui/message` only after prepare succeeds. The process-local binding id fences duplicate/reloaded iframes but grants no authority and is not durable execution truth; the Store retains only an identity-bound SHA-256 fingerprint of the current MCP App binding as restart provenance.

Every bridge operation re-runs ordinary communication authorization and exact Agent/Endpoint/controller-generation validation. Ordinary push bindings still require an Endpoint freshly attached in the current Server process. For MCP Apps only, successful bind/replacement persists the current identity-bound recovery fingerprint; normal unbind, detach, expiry, Endpoint replacement, or transition to a push carrier clears it. Server takeover clears process-local bindings and `wake_capable` but preserves that fingerprint. With no local binding, only the exact fingerprint-proven View receives a `success=true` state projection with `host_binding.bound=false` and `recovery.kind=host_binding_missing_in_process`; bind may then recreate that same process-local carrier. The strict published continuation projection schema requires `recovery` on every result (`null` normally, that sole fixed object for proven restart loss), so Host schema projection cannot discard the recovery observation. This avoids relying on a failed ToolResult that ChatGPT may collapse to JSON-RPC `-32000`. A different/replaced View remains `host_binding_stale`, and stale generation, expiry, detach, authorization, malformed result, or generic bridge failures never enter restart recovery. Replacing or withdrawing a View reuses existing wake-capability reconciliation: a pre-fence claim is revoked and the logical Wake returns to `pending`, while a prepared/delivered Attempt becomes `delivery_unknown`. The App never blindly resends after the dispatch fence. Host `ui/message` success means only `dispatch_accepted`; only later exact `consume_agent_wake` proves that a continuation model turn actually ran. A consume-before-ACK race is valid and late ACK is idempotent. Hidden/background Views heartbeat but do not initiate a new automatic Host dispatch, so Host scheduling remains best effort/non-immediate.
