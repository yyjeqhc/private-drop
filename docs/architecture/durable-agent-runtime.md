...
- process-local Host bindings are empty after restart; offline Messages, Deliveries,
  ...
  register a carrier, and an already-open MCP App may recreate only its lost local
  binding for the same still-current exact Endpoint generation after takeover.
