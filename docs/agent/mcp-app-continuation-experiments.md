...
The only card-creating entry is the explicit read `present_agent_continuation(agent_id, endpoint_id, expected_controller_generation)`, bound to the current `ui://webcodex/agent-continuation/v12` resource. Bind/state/acquire/prepare/finish/unbind are globally ModelHidden and are projected only as App-visible tools on eligible Stateless MCP 2026 operator surfaces. They do not bind the resource again, so polling/coordination does not create a stream of custom cards. Ordinary communication and coding tools keep native Host presentation.
...
`ui://webcodex/agent-continuation/v12` and `ui://webcodex/goal-plan/v2` resources.
Agent continuation v1-v11 are hidden read aliases serving the same current template;
they do not revive expired/stale Endpoints or bypass exact generation/authorization fencing. The v11 fingerprint-proven restart-recovery authority contract remains unchanged; v12 makes its successful recovery observation part of the strict published projection schema.
