...
Desktop prepares local projects and manages connections. You ask for work in ChatGPT or another AI client. For installation, Tunnel environment variables, and system permissions, see the [installation and connection guide](desktop-install.md).
...
3. Tunnel presence checks live under **Optional: check ChatGPT tunnel configuration**. You can prepare the local project before configuring a Tunnel. If configuration is present, you can opt into connecting ChatGPT after setup.
...
Connection shows current status and available controls first, followed by ChatGPT handoff instructions. Missing Tunnel configuration automatically expands presence checks. Checks never display the API key.

| Situation | Next action |
| --- | --- |
| Local runtime is not ready | Restore it on Home; reconfigure the same project if necessary |
| Tunnel ID or API key is missing | Follow the installation guide, fully quit and reopen Desktop, then recheck |
...
**Settings** contains language, launch at login, Tunnel networking, and diagnostics. You can also switch language at the bottom of the sidebar.
